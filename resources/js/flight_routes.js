/* document.addEventListener("DOMContentLoaded", function() {
    ...
    var routeInput = document.querySelector('input[name="route"]');
    ...
    function actualizarRuta() {
        var origenIndex = originSelect.selectedIndex;
        var destinoIndex = destinationSelect.selectedIndex;

        // Verificar si se seleccionó una opción válida en ambos selectores
        if (origenIndex !== 0 && destinoIndex !== 0) {
            var origen = originSelect.options[origenIndex].text;
            var destino = destinationSelect.options[destinoIndex].text;

            // Verificar si las ciudades seleccionadas son diferentes
            if (origen !== destino) {
                var ruta = concatenarRuta(origen, destino);
                routeInput.value = ruta;
                ocultarAdvertencia();
                addButton.disabled = false; // Habilitar el botón de añadir
            } else {
                mostrarAdvertencia("¡Por favor, selecciona ciudades diferentes!");
                routeInput.value = "";
                addButton.disabled = true; // Deshabilitar el botón de añadir
            }
        } else {
            ocultarAdvertencia();
            routeInput.value = "";
    ...
    originSelect.addEventListener("change", actualizarRuta);
    destinationSelect.addEventListener("change", actualizarRuta);
